...
| `SearchCallBudgetMiddleware` | `search_recipes`/`search_delivery_menu`/`search_web_food` 호출 횟수를 대화당 최대 8회로 강제 제한 — 프롬프트 지시("최대 5회")만으로는 모델이 지키지 않아, 한도 초과 시 실제 재실행 없이 "지금 있는 결과로 최종 응답을 완성하라"는 지시로 대체해 응답 미완성 상태로 반복되는 것을 방지 |
...
- **레시피 데이터 규모와 Tool 호출 수렴 실패**: 레시피 20개가 대부분 200~650kcal대라, 목표 칼로리가 높을수록(특히 유지/증량처럼 감량보다 목표치가 큰 경우) 여러 요리를 조합해야 해서 agent의 검색이 오래 걸립니다. 실측 중 다음 문제들을 순서대로 발견하고 대응했습니다: (1) `temperature=0`에서 동일한 Tool을 동일한 인자로 무한 반복 호출하는 결정적 루프 → `DuplicateToolCallGuardMiddleware`로 차단, (2) 반복은 막혀도 모델이 계속 새 조합을 탐색하며 끝내 응답을 완성하지 못하는 경우 → `SearchCallBudgetMiddleware`로 검색류 Tool 호출을 대화당 8회로 강제 제한(프롬프트의 "최대 5회" 지시만으로는 모델이 지키지 않아 기계적 강제가 필요했음), (3) RAG에 없는 고칼로리/특수 메뉴 보완을 위해 `search_web_food`(Tavily 웹 검색 기반) Tool 추가. 이 조합으로 실패율이 크게 낮아졌지만(반복 실측 중 무한 루프성 실패가 대부분 해소됨), 목표 칼로리가 유난히 큰 일부 프로필에서는 여전히 낮은 확률로 `recursion_limit`(60)에 도달해 "조건에 맞는 메뉴를 찾기 어렵다"는 안내로 대체되는 경우가 남아있습니다(`GraphRecursionError`를 안전하게 처리하므로 무한 대기나 과도한 비용으로 이어지지는 않음). 완전한 해결은 고칼로리 레시피 데이터 추가나 더 큰 모델(gpt-4o 등)로의 교체가 필요해 향후 과제로 남아있습니다.
